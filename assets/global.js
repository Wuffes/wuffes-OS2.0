...
class ScrollButton extends HTMLElement {
  constructor() {
    ...
    this.icon = this.querySelector('.scroll-button--link');
  ...
  onButtonClick(event) {
    ...
    const targetLink = event.target.getAttribute('data-target-id');
    const headerHeight = document.querySelector('header.header').offsetHeight + 15;

    if (targetLink) {
      const targetElement = document.getElementById(targetLink);
      if (targetElement) {
        const targetY =
          (targetElement.getBoundingClientRect().top + window.scrollY || window.pageYOffSet) - headerHeight;
        this.animateScroll(targetY, 333);
      }
    }
  }
